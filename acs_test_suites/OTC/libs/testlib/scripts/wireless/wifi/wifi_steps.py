...
class open_wifi_settings(wifi_step):
    ...
    def do(self):
        ...
        if self.use_adb is True:
            ...
            time.sleep(5)
            ui_steps.click_button_common(serial=self.serial, view_to_find={
                                         "textMatches": "Wi.Fi"}, view_to_check={"textMatches": "Wi.Fi"})()
            ...
            for param_name in self.params.keys():
                if self.params[param_name] is not None:
                    if self.wifi_info[param_name] == "UNKNOWN/IDLE":
                        ...
                        m = re.search(
                            self.params[param_name], self.wifi_info[param_name])
...
class set_wifi_state(wifi_step):
    ...
    def do(self):
        ...
        if self.open_wifi_settings:
            open_wifi_settings(serial=self.serial, use_adb=self.use_adb)()
...
class wait_until_connected(wifi_step):
    ...
    def do(self):
        self.resolution = wait_for_state(
            serial=self.serial, state=self.connected, timeout=self.timeout)()
...
class scan_and_check_ap(wifi_step):
    ...
    def __init__(self, ap, option="set_off", trycount=10, open_wifi=None, should_exist=True, **kwargs):
        ...
        if self.should_exist:
            ...
            self.set_errorm(
                "", "AP SSID was found by scan when it should not: " + self.ap)
        ...
        while not self.found_ap and iteration < self.trycount:
            if self.uidevice(scrollable=True).exists:
                self.found_ap = self.uidevice(
                    scrollable=True).scroll.to(text=self.ap)
...
class clear_saved_networks(wifi_step):
    ...
    def __init__(self, max_entries=20, use_adb=False, open_wifi_setting=True, **kwargs):
        wifi_step.__init__(self, **kwargs)
    ...
    def do(self):
        if self.open_wifi_settings:
            ...
            set_wifi(serial=self.serial, state="ON", use_adb=self.use_adb)()
            open_wifi_settings(serial=self.serial, use_adb=self.use_adb)()
...
class set_wifi_security(wifi_step):
    ...
    def __init__(self, security, password=None, EAP_method=None, phase_2_auth=None,
                 user_certificate=None, identity=None,
                 anonymous_identity=None, ca_certificate=None,
                 clear_text_boxes=True, **kwargs):
        wifi_step.__init__(self, **kwargs)
        self.security = security
...
class install_WIFI_certificate(wifi_step):
    ...
    def do(self):
        ...
        if self.uidevice(className="android.widget.ListView", scrollable=True).wait.exists(timeout=self.wait_time):
            self.uidevice(scrollable=True).scroll.to(
...
class add_network(wifi_step):
    ...
    def __init__(self, ssid, security, password=None, EAP_method=None, phase_2_auth=None,
                 user_certificate=None, ca_certificate=None, identity=None, anonymous_identity=None,
    ...
    def do(self):
        self.wrap_do()
        try:
            set_wifi_security(serial=self.serial, security=self.security, password=self.password,
            ...
            install_WIFI_certificate(
                serial=self.serial, cert_pass="whatever", cert_name="TLS_certificate", dut_pin=1234)()
            self.wrap_do()
        ...
        # Click Save
        apply_config_btn = {
            "className": "android.widget.Button", "enabled": True}
        ...
        if self.apply_config:
            if self.uidevice(resourceId="android:id/title", text=self.ssid).wait.exists(timeout=1000):
                self.uidevice(resourceId="android:id/title",
...
class modify_network(wifi_step):
    ...
    def __init__(self, ssid, password=None, proxy=None, proxy_pac_url=None,
                 proxy_hostname=None, proxy_port=None, proxy_bypass=None,
                 ip_settings=None, ip_address=None, gateway=None,
                 network_prefix_length=None, dns1=None, dns2=None,
                 valid_config=True, **kwargs):
        wifi_step.__init__(self, **kwargs)
        self.ssid = ssid
        self.password = None
        self.proxy = proxy
        ...
        self.advanced = (self.proxy or self.ip_settings) is not None
        self.ssid_view = {"textContains": self.ssid,
                          "resourceId": "android:id/title"}
        self.modify_network_view = {
            "textContains": "Modify network", "resourceId": "android:id/title"}
...
class ping_gateway(wifi_step):
    ...
    def do(self):
        wifi_conf = self.adb_connection.parse_cmd_output("dumpsys wifi")
        self.wifi_info = wifi_utils.get_connection_info(wifi_conf)
        (self.status, loss, ping_output) = wifi_utils.ping(ip=self.wifi_info['Gateway'],
...
class ping_ip(wifi_step):
    ...
    def do(self):
        if self.ip is not None:
            ...
            step_message = 'Ping failure due to invalid destination IP address! Check device connectivity! IP is: ' + \
                str(self.ip)
            self.set_errorm("", step_message)
            self.status = False
    ...
    def do(self):
        ...
        if self.ip_range:
            ...
            while not found and current_ip != end_ip:
                if current_ip.split('.')[-1] != '255' and current_ip.split('.')[-1] != '0' \
                        and current_ip.split('.')[-1] != '1':
