...
class dump(ui_step):
    ...
    def do(self):
        if self.out_file:
            self.uidevice.dump(out_file=self.out_file, compressed=False, serial=self.serial)
        else:
...
class set_pin_screen_lock(ui_step):
    ...
    def do(self):
        ...
        click_button(serial=self.serial, view_to_find={"textContains": "Screen lock"})()
        if self.uidevice(text="Confirm your PIN").wait.exists(timeout=self.wait_time):
            if self.device_info.confirm_pin_go_back is not None:
                click_button(serial=self.serial, view_to_find=self.device_info.confirm_pin_go_back)()
            else:
            ...
            click_button(serial=self.serial, view_to_find={"textContains": "PIN"})()
            if self.uidevice(text="Require PIN to start device").wait.exists(timeout=self.wait_time):
                if self.require_pin_to_start_device:
                    click_button(serial=self.serial, view_to_find={"textContains": "Require PIN to start device"})()
                    click_button(serial=self.serial, view_to_find={"text": "OK"}, optional=True)()
                else:
                    click_button(serial=self.serial, view_to_find={"textContains": "No thanks"})()
                if self.device_info.dessert == "M":
                    click_button(serial=self.serial, view_to_find={"textContains": "Continue"})()
            if self.uidevice(text="Secure start-up").wait.exists(
                    timeout=self.wait_time):
                if self.require_pin_to_start_device:
                    click_button(serial=self.serial, view_to_find={"text": "YES"}, optional=True)()
                else:
                    click_button(serial=self.serial, view_to_find={"text": "NO"}, optional=True)()
            ...
            click_button(serial=self.serial, view_to_find={"resourceId": "com.android.settings:id/next_button"})()
            edit_text(serial=self.serial, view_to_find={"resourceId": "com.android.settings:id/password_entry"},
                      value=self.dut_pin, is_password=True)()
            click_button(serial=self.serial, view_to_find={"resourceId": "com.android.settings:id/next_button"})()
            click_button(serial=self.serial, view_to_find=self.device_info.password_done_btn_id)()
...
class remove_pin_screen_lock(ui_step):
    ...
    def do(self):
        ...
        click_button(serial=self.serial, view_to_find={"textContains": self.new_mode})()
        ...
        if self.uidevice(textContains=self.device_info.remove_pin_confirm_desc).wait.exists(timeout=self.wait_time):
            click_button(serial=self.serial, view_to_find={"text": self.device_info.remove_pin_confirm_button})()
...
class am_start_command(adb_step):
    ...
    def check_condition(self):
        ...
        if self.view_presence:
            self.step_data = self.uidevice(**self.view_to_check).wait.exists(timeout=self.timeout)
        else:
            self.step_data = self.uidevice(**self.view_to_check).wait.gone(timeout=self.timeout)
        return self.step_data
...
class scroll_up_to_view(ui_step):
    ...
    def do(self):
        ...
        while (not self.uidevice(**self.view_to_check).wait.exists(timeout=1000) and iterations < self.iterations):
            swipe(serial=self.serial, sx=self.start_x, sy=self.start_y, ex=self.end_x, ey=self.end_y, steps=10)()
            iterations += 1
        ...
        if self.view_to_check:
            while iterations < self.iterations:
                if not self.uidevice(**self.view_to_check).wait.exists(timeout=1000):
                    self.uidevice.swipe(self.start_x, self.start_y, self.end_x, self.end_y, self.steps)
                iterations += 1
        else:
            self.uidevice.swipe(self.start_x, self.start_y, self.end_x, self.end_y, self.steps)
...
class open_app_from_allapps(ui_step):
    ...
    def do(self):
        ...
        ui_utils.click_apps_entry(serial=self.serial, view_to_find=self.view_to_find)
...
class open_settings(ui_step):
    ...
    def do(self):
        if self.intent:
            am_start_command(serial=self.serial, component="com.android.settings/.Settings")()
        else:
            ...
            if all_apps_icon is not None:
                ...
                #        break
                open_app_from_allapps(serial=self.serial, view_to_find={"text": "Settings"})()
            else:
...
class open_settings_app(ui_step):
    ...
    def do(self):
        ...
        open_app_from_settings(serial=self.serial, view_to_find=self.view_to_find)()
...
class open_app_from_settings(ui_step):
    ...
    def do(self):
        if self.uidevice(scrollable=False).exists:
            self.uidevice(scrollable=False).scroll.vert.toBeginning(steps=100, max_swipes=1000)
...
class open_playstore(ui_step):
    ...
    def do(self):
        open_app_from_allapps(serial=self.serial, view_to_find={"text": "Play Store"})()

    def check_condition(self):
        self.uidevice.wait.idle()
        return self.uidevice(text="Add a Google Account").wait.exists(timeout=5000) or \
            self.uidevice(text="HOME", resourceId="com.android.vending:id/title").wait.exists(timeout=20000)
...
class open_google_books(ui_step):
    ...
    def do(self):
        open_app_from_allapps(serial=self.serial, view_to_find={"text": "Play Books"})()
...
class add_google_account(ui_step):
    ...
    def do(self):
        ...
        if self.version == "L":
            ...
            click_button_common(serial=self.serial, view_to_find={"text": "Add account"})()

            click_button_common(serial=self.serial, view_to_find={"text": "Google"})()

            edit_text(serial=self.serial, view_to_find={"text": "Email or phone"}, value=self.account)()

            click_button_common(serial=self.serial, view_to_find={"text": "NEXT"})()
            ...
            click_button_common(serial=self.serial, view_to_find={"text": "I AGREE"})()
            # todo: check what else can be checked

            click_button_common(serial=self.serial,
                                view_to_find={"resourceId": "com.google.android.gms:id/next_button"},
        ...
        if self.version != "O":
            ...
            click_button(serial=self.serial, view_to_find={"text": "Existing"}, view_to_chec={"text": "Sign in"})()

            edit_text(serial=self.serial,
                      view_to_find={"resourceId": "com.google.android.gsf.login:id/username_edit"},
                      value=self.account)()
            edit_text(serial=self.serial,
                      view_to_find={"resourceId": "com.google.android.gsf.login:id/password_edit"},
                      value=self.password, is_password=True)()

            click_button(serial=self.serial,
                         view_to_find={"resourceId": "com.google.android.gsf.login:id/next_button"},
                         view_to_check={"text": "OK"})()

            click_button(serial=self.serial, view_to_find={"text": "OK"}, wait_time=99999,
                         view_to_check={"text": "Google services"})()

            click_button(serial=self.serial, view_to_find={"descriptionContains": self.device_info.next_btn_id})()

        if self.version == "L":
            self.uidevice(textContains="Set up payment").wait.exists(timeout=3000)
            click_button(serial=self.serial, view_to_find={"textContains": "Skip"}, wait_time=3000,
                         view_to_check={"text": "Google"})()
        elif self.version == "K":
            self.uidevice(serial=self.serial, text="Not now").wait.exists(timeout=3000)
...
class add_app_from_all_apps_to_homescreen(ui_step):
    ...
    def do(self):
        ...
        self.x_coord = (app.info['bounds']['left'] + app.info['bounds']['right']) / 2
        self.y_coord = (app.info['bounds']['bottom'] + app.info['bounds']['top']) / 2
        swipe(serial=self.serial, sx=self.x_coord, sy=self.y_coord, ex=self.x_coord, ey=self.y_coord)()
...
class uninstall_app_from_apps_settings(ui_step, base_step):
    ...
    def do(self):
        ...
        click_button(serial=self.serial, view_to_find={"textContains": "Uninstall"}, view_to_check={"text": "OK"})()
...
class uninstall_app(ui_step):
    ...
    def do(self):
        open_settings(serial=self.serial)()
        open_app_from_settings(serial=self.serial, view_to_find={"text": "Apps"}, view_to_check="Donwloaded")()
        click_button(serial=self.serial, view_to_find={"text": self.app_name},
                     view_to_check={"textContains": "Uninstall"})()
        click_button(serial=self.serial, view_to_find={"textContains": "Uninstall"}, view_to_check={"text": "OK"})()
        click_button(serial=self.serial, view_to_find={"text": "OK"}, view_to_check={"text": "OK"},
    ...
    def do(self):
        ...
        if (is_checked and self.state == "OFF") or (not is_checked and self.state == "ON"):
            ...
            if self.confirm_view and self.uidevice(**self.confirm_view).wait.exists(timeout=self.wait_time):
                click_button(serial=self.serial, view_to_find=self.confirm_view, view_to_check=self.view_to_check)()
...
class open_picture_from_gallery(ui_step, adb_step):
    ...
    def do(self):
        open_app_from_allapps(serial=self.serial, view_to_find={'text': 'Gallery'})()
...
class enable_developer_options(ui_step):
    ...
    def do(self):
        if self.intent:
            ...
            open_settings(serial=self.serial)()
            click_button_common(serial=self.serial, view_to_find={"text": "System"}, optional=True)()
            click_button_common(serial=self.serial, view_to_find={"textContains": "About "},
                                view_to_check={"text": "Build number"})()
            for i in range(7):
                click_button_common(serial=self.serial, view_to_find={"text": "Build number"})()
            press_back(serial=self.serial)()
...
class disable_options_from_developer_options(ui_step):
    ...
    def do(self):
        ...
        version = adb_utils.get_android_version(serial=self.serial, full_version_name=True)
        is_switch = True
        if version == "5.0":
            is_switch = False
...
class unlock_device_pin(ui_step):
    ...
    def do(self):
        ...
        click_button(serial=self.serial, view_to_find={"descriptionContains": "Enter"})()
    ...
    def do(self):
        ...
        if not self.uidevice(textContains=self.device_info.predefined_language_text_id)\
            ...
            if not self.uidevice(textContains=self.device_info.predefined_language_text_id)\
                ...
                click_button(serial=self.serial, view_to_find={"textContains": "United State"})()
        if self.device_info.dessert == "M":
            click_button(serial=self.serial, view_to_find={"textContains": "United State"})()
        ...
        click_button(serial=self.serial, view_to_find=self.device_info.wifi_skip_anyway_btn_id)()
        ...
        click_button(serial=self.serial, view_to_find=self.device_info.skip_anyway_btn_id)()
        ...
        click_button(serial=self.serial, view_to_find=self.device_info.finish_startup_btn_id)()
    ...
    def do(self):
        ...
        if self.uidevice(textContains="SIM").wait.exists(timeout=1000):
            click_button(serial=self.serial, view_to_find={"text": "Skip"}, wait_time=10000,
                         view_to_check={"textContains": "network"})()
        click_button(serial=self.serial, view_to_find={"text": "Skip"}, view_to_check={"text": "Skip anyway"})()

        click_button(serial=self.serial, view_to_find={"text": "Skip anyway"}, view_to_check={"text": "Name"})()

        edit_text(serial=self.serial, view_to_find={"className": "android.widget.EditText"}, value=self.user_name)()
        click_button(serial=self.serial, view_to_find={"text": self.device_info.next_btn_id})()
...
class set_orientation(ui_step):
    ...
    def do(self):
        self.uidevice.orientation = self.__orientation[self.target][self.orientation]
        time.sleep(1)
...
class scroll_to(ui_step):
    ...
    def do(self):
        self.success = self.uidevice(scrollable=True).scroll.to(**self.view_to_find)
...
class remove_google_account(ui_step):
    ...
    def check_condition(self):
        ...
        close_app_from_recent(serial=self.serial, view_to_find={"text": "Settings"})()
...
class close_all_app_from_recent(ui_step):
    ...
    def do(self):
        ...
        self.uidevice(resourceId="com.android.systemui:id/task_view_content").wait.exists(timeout=20000)
        while(self.uidevice(resourceId="com.android.systemui:id/task_view_content").count > 0):
                self.uidevice(resourceId="com.android.systemui:id/task_view_content").swipe.right()

    def check_condition(self):
        press_home(serial=self.serial)()
        press_recent_apps(serial=self.serial)()
        exist_stat = self.uidevice(text="Your recent screens appear here").wait.exists(timeout=1000)
        press_home(serial=self.serial)()
        return exist_stat
...
class set_timezone_from_settings(ui_step):
    ...
    def do(self):
        ...
        if not self.uidevice(text=self.timezone).wait.exists(timeout=1000):
            scroll_up_to_view(serial=self.serial, view_to_check={"text": self.timezone}, ey=100, critical=False)()
        if not self.uidevice(text=self.timezone).wait.exists(timeout=1000):
            scroll_up_to_view(serial=self.serial, view_to_check={"text": self.timezone}, ey=500, iterations=20)()
...
class set_orientation_vertical(ui_step):
    ...
    def do(self):
        ...
        set_orientation(serial=self.serial, orientation="portrait", target="phone")()
        # set the orientation to 'portrait'
        self.device_type = adb_utils.get_device_orientation_type(serial=self.serial)
        set_orientation(serial=self.serial, orientation=self.orientation, target=self.device_type)()
...
class add_trusted_location(ui_step):
    ...
    def do(self):
        ...
        click_button(serial=self.serial, view_to_find={"text": "Add trusted place"})()
        ...
        if self.device_info.dessert == "M":
            ...
            if ui_utils.is_display_direction_landscape(serial=self.serial):
                click_xy(serial=self.serial, x=self.uidevice.info["displayWidth"] / 2,
                         y=self.uidevice.info["displayHeight"] * self.device_info
...
class add_trusted_device(ui_step):
    ...
    def do(self):
        ...
        click_button(serial=self.serial, view_to_find={"text": self.device_name}, view_to_check={"text": "YES, ADD"})()
        click_button(serial=self.serial, view_to_find={"text": "YES, ADD"}, view_to_check={"text": "Connected"})()
...
class set_date_and_time(ui_step):
    ...
    def do(self):
        ...
        for i in range(len(views_to_click)):
            click_button(serial=self.serial, view_to_find=views_to_click[i], view_to_check=views_to_check[i])()
...
class press_map(ui_step):
    ...
    def do(self):
        if self.device_info.platform not in ["gordon_peak"]:
            ...
            map_x_coordinate = activity_bar_single_element_width * 2 - activity_bar_single_element_width / 2
...
class press_dialer(ui_step):
    ...
    def do(self):
        if self.device_info.platform not in ["gordon_peak"]:
            ...
            dialer_x_coordinate = activity_bar_single_element_width * 3 - activity_bar_single_element_width / 2
    ...
    def check_condition(self):
        if self.step_data is True:
            self.step_data = wait_for_view(serial=self.serial,
                                           view_to_find={"text": "Phone", "packageName": "com.android.car.dialer"},
                                           timeout=self.timeout)()
        return self.step_data
...
class press_media(ui_step):
    ...
    def do(self):
        if self.device_info.platform not in ["gordon_peak"]:
            ...
            media_x_coordinate = activity_bar_single_element_width * 5 - activity_bar_single_element_width / 2
...
class click_button_common(ui_step):
    ...
    def check_condition(self):
        self.set_errorm(self.error_trace_msg, "Could not click {0} checking {1}".format(self.view_to_find,
                                                                                        self.view_to_check))
        self.set_passm("{0} clicked checking {1}".format(self.view_to_find, self.view_to_check))
        if self.step_data is False and self.optional:
            self.set_passm("Could not click {0} and skipped as " "optional step".format(self.view_to_find))
            return True
...
class wait_for_view_common(ui_step):
    ...
    def do(self):
        ...
        if self.second_view_to_find is not None:
            if self.position is not None:
                ...
                try:
                    if uobj(**self.second_view_to_find).exists:
                        self.step_data = True if not self.retrieve_info else uobj(**self.second_view_to_find).info
                except Exception:
    ...
    def check_condition(self):
        if self.second_view_to_find is None:
            if self.view_presence:
                self.set_errorm(self.error_trace_msg, "Could not find {" "0}".format(self.view_to_find))
                self.set_passm("Found view {0}".format(self.view_to_find))
            else:
                self.set_errorm(self.error_trace_msg, "View {0} still " "exists".format(self.view_to_find))
            ...
            if self.view_presence:
                ...
                self.set_passm("View {0}''\,,{1} is gone".format(self.view_to_find, self.second_view_to_find))
...
class search_image_object(ui_step):
    ...
    def do(self):
        ...
        try:
            ...
            self.logger.info("Unable to remove temp file: {}".format(temp_file_name))
