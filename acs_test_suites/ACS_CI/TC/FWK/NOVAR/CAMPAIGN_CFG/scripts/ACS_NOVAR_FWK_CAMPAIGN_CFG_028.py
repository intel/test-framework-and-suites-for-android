...
from acs.UseCase.Misc.EXEC_SCRIPT import ExecScript

OUTPUT = ""
VERDICT = SUCCESS

if not EXEC_UC.get_usecase_name() == "MY_CUSTOM_UC_NAME":
    VERDICT = FAILURE
    OUTPUT = "UseCase name should be %s instead of %s" % ("MY_CUSTOM_UC_NAME", EXEC_UC.get_usecase_name())
else:
    if not isinstance(EXEC_UC, ExecScript):
        VERDICT = FAILURE
        OUTPUT = "UseCase class should be ExecScript instead of %s" % (type(EXEC_UC))
    else:
