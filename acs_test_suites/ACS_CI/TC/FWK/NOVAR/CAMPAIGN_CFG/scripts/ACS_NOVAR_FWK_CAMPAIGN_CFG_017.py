...
b2b_iter = EXEC_UC.get_b2b_iteration()
isContinuous = EXEC_UC.get_b2b_continuous_mode()
(max_attempts, acceptance) = EXEC_UC.get_acceptance_criteria()
expected = EXEC_UC.get_tc_expected_result()
is_critical = EXEC_UC.get_is_critical()

if (b2b_iter == 2
    and isContinuous is True
    and max_attempts == 2
    and acceptance == 2
    and expected == Util.Verdict.FAIL
        and is_critical is True):
    VERDICT = FAILURE
elif expected == Util.Verdict.PASS:
    VERDICT = FAILURE
else:
    VERDICT = SUCCESS  # error : test expected to fail
