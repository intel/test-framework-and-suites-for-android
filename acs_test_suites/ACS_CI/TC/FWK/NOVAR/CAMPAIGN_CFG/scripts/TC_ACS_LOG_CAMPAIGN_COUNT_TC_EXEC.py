...
# Optional parameter
__tc_number_execution_expected = int(TC_PARAMETERS("PREVIOUS_TC_EXECUTED_COUNT_EXPECTED"))  # @UndefinedVariable
# PRINT_INFO("Number of test case executed EXPECTED : %s" % tc_number_execution_expected)  # @UndefinedVariable

# Open ACS log file and count the number of TestCase occurence
with open(ACS_REPORT_FILE_PATH, 'r') as f:
    tree = ElementTree.parse(f)

__tc_number_execution_done = len(tree.findall('//TestCase'))

if __tc_number_execution_done == __tc_number_execution_expected:
    VERDICT = SUCCESS
else:
    VERDICT = FAILURE

OUTPUT = "Number of test case executed EXPECTED : " + \
    str(__tc_number_execution_expected) + " - EXECUTED : " + str(__tc_number_execution_done)  # @UndefinedVariable
