...
from MY_OWN_UC import My_UC
VERDICT = SUCCESS

OUTPUT = ""
if not EXEC_UC.get_usecase_name() == "MY_OWN_UC_NAME":
    VERDICT = FAILURE
    OUTPUT = "UseCase name should be %s instead of %s" % ("MY_OWN_UC_NAME", EXEC_UC.get_usecase_name())
else:
    if not isinstance(EXEC_UC, My_UC):
        VERDICT = FAILURE
        OUTPUT = "UseCase class should be My_UC instead of %s" % (type(EXEC_UC))
    else:
        if getattr(EXEC_UC, "my_own_method", None) is not None and EXEC_UC.my_own_method() == "result expected":
            OUTPUT = "UseCase Class override has worked!"
        else:
            VERDICT = FAILURE
            OUTPUT = "UseCase class should be My_UC instead of %s" % (type(EXEC_UC))


MY_OWN_UC = 'MY_OWN_UC.py'
if os.path.exists(MY_OWN_UC):
    os.remove(MY_OWN_UC)
